...
- [ ] 1.1 为最近播放补齐同一快照定位、目标失效、100 条边界和会话不重排的表征测试
- [ ] 1.2 为快速选播 Adapter 补齐 selectAt、非法位置、stable ID、当前项高亮和缩略图局部更新测试
- [ ] 1.3 为 BGM 目标动作建立纯 Kotlin 决策矩阵测试，覆盖媒体类型、用户播放意图、生命周期和配置组合
- [ ] 1.4 为快速连续选择媒体补充旧请求晚到、取消和预加载不改当前项的并发测试
- [ ] 1.5 运行完整基线门禁并记录测试数量、release APK SHA-256、首个视频和大文件真机基线

## 2. 修复已确认状态缺陷

- [ ] 2.1 添加“图片暂停同步暂停 BGM、暂停状态返回前台不恢复”的失败测试
- [ ] 2.2 修改 BGM 播放条件，使其同时受图片类型、isPlaying 和前台生命周期约束
- [ ] 2.3 添加“焦点项与当前项不同但只有当前项显示底部蓝条”的失败测试
- [ ] 2.4 修改快速选播视觉绑定，使完整边框只表示焦点、底部蓝条只表示当前播放
- [ ] 2.5 添加“历史目标不存在时不播放其他项”的失败测试并改为同一历史快照定位
- [ ] 2.6 添加“BGM 解析失败可重试且 CancellationException 透明传播”的失败测试并修复现有解析状态
